...
int run(t_info *info)
{
	...
	while (info->exit == 0)
	{
		...
		// cd handling
		if ((ft_strncmp(info->set_list->data->data, "cd", ft_strlen(info->set_list->data->data))) == 0)
			handle_cmd_cd(info->set_list->data, info->env_list, &(info->old_pwd));
		if ((ft_strncmp(info->set_list->data->data, "pwd", ft_strlen(info->set_list->data->data))) == 0)
			handle_cmd_pwd();
//		break;
